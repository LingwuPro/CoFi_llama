...
def run():
    ...
    for data in tqdm(dev_dataset, total=len(dev_dataset)):
        answer.append({"answer" : evaluate(model, data['instruction'], input=data['input'])})

    print(answer)

    with open('./answer.jsonl', 'w') as json_file:
        json_file.write(json.dumps(answer, ensure_ascii=False, indent=4))
